      ...
      <React.Fragment>
          ...
          {event &&
            ...
            event.event.guest && (
              <FieldContainer margin="0">
                <FieldLabel>Guests ({event.event.guests.length})</FieldLabel>
              </FieldContainer>
            )}
          <FieldsRow>
            <FieldContainer>
              {event.event &&
                event.event.is_master_code_enabled && (
                  <p>
                    Master Code: <b>{event.event.master_code}</b>
                  </p>
                )}
            </FieldContainer>
            <FieldContainer align="flex-end">
              <FieldsRow>
                <FieldContainer>
                  <ImportGuestList {...this.props} />
                </FieldContainer>
                <FieldContainer>
                  <EventInviteGuest {...this.props} />
                </FieldContainer>
              </FieldsRow>
            </FieldContainer>
          </FieldsRow>
        ...
        <TableContainer>
          ...
          event.event.guests.length > 0 ? (
            <Panel
              shaded
              style={{ backgroundColor: 'white', margin: '1em 0 0 0' }}
            >
              <Table
              ...
              >
                <Column flexGrow>
                  <HeaderCell>LiquidIntel Member?</HeaderCell>
                ...
                <Column resizable>
                  ...
                  <Cell dataKey="email">{rowData => rowData.email}</Cell>
                </Column>
                <Column flexGrow>
                ...
                <Column flexGrow>
                  <HeaderCell>Guest Type</HeaderCell>
                  <Cell dataKey="phone_number">
                    {rowData => rowData.role.name}
                  </Cell>
                </Column>
                <Column flexGrow>
                  <HeaderCell>Signup Code</HeaderCell>
                  <Cell dataKey="code">
                    {rowData => (
                      <b>{rowData.account_id ? '-' : rowData.code}</b>
                    )}
                  </Cell>
                </Column>
                <Column flexGrow>
                    ...
                    {rowData => (
                      <div>
                        {rowData.email &&
                          !rowData.account_id && (
                            <a
                              onClick={() => this.handleResendEmail(rowData.id)}
                            >
                              Send email
                            </a>
                          )}
                        {rowData.email && !rowData.account_id && ` | `}
                        <a onClick={() => this.handleDelete(rowData.id)}>
                          Revoke
                        </a>
                      </div>
                    )}
            ...
            <FieldContainer>No guests</FieldContainer>
          )}
        </TableContainer>
      </React.Fragment>
    );

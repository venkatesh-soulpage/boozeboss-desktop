...
const VerificationPicture = props => (
  <div>
    {props.verification_attachment.verification_type === 'id_front' && (
      <h4>ID Front side</h4>
    )}
    {props.verification_attachment.verification_type === 'id_back' && (
      <h4>ID Back side</h4>
    )}
    {props.verification_attachment.verification_type === 'selfie' && (
      <h4>Selfie</h4>
    )}
    <Divider />
        ...
        {verification ? (
          <Panel bordered>
            <DataContainer>
              {verification &&
                ...
                verification.verifications.map(verification_attachment => {
                  return (
                    <VerificationPicture
                      verification_attachment={verification_attachment}
                    />
                  );
                })}
            </DataContainer>
            <VerificationRow>
              <StyledButton
                color="green"
                onClick={() => this.handleVerification('APPROVED')}
              >
                Verify
              </StyledButton>
              <StyledButton
                color="red"
                onClick={() => this.handleVerification('REJECTED')}
              >
                Reject
              </StyledButton>
            </VerificationRow>
